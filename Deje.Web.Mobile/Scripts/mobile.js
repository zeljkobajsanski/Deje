...
function prikaziArtikle(data) {
    ...
    $.each(data, function(i, e) {
        var $li = $("<li></li>");
        var $a = $("<a href='#Mapa?tip=" + e.Tip + "&id=" + e.Id + "'>" + "<span class='ui-li-count'>" + e.Broj + "</span>" + "</a>");
...
function prikaziNaMapi() {
    if ($.mobile.pageData && $.mobile.pageData.tip && $.mobile.pageData.id) {
        prikaziCekalicu();
        ...
        alert("Parametri nisu prosleđeni");
    }
...
function getInfoboxContent(dobavljac) {
    var navigateTo = "window.location='#Dobavljac?id=" + dobavljac.Id + "'";
...
$(function () {
    ...
    $("#Mapa").live('pageinit', function () {
        ...
        infoboxEntities = new Microsoft.Maps.EntityCollection({ visible: true });
        map.entities.push(infoboxEntities);
    });
